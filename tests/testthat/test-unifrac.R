...
test_that("Unweighted UniFrac works", {
  expect_equal(
    UniFrac(physeq, weighted = FALSE, normalized = FALSE)[1],
    3.12
  )
  expect_equal(
    UniFrac(physeq, weighted = FALSE, normalized = TRUE)[1],
    3.12 / 8.45
  )
})
...
test_that("Weighted UniFrac works", {
  expect_equal(
    UniFrac(physeq, weighted = TRUE, normalized = FALSE)[1],
    1.54343434343434
  )
  expect_equal(
    UniFrac(physeq, weighted = TRUE, normalized = TRUE)[1],
