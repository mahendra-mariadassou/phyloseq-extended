...
physeq = phyloseq(otu_table(table, taxa_are_rows = TRUE),
                  phy_tree(tree))
...
test_that("unifrac fails when the tree is missing, unrooted or lacks branch lengthes", {
  ...
  expect_error(unifrac(physeq), "Unifrac distances require a rooted tree, consider rooting your tree. See ?ape::root.", fixed = TRUE)
  ## Missing branch lengthes
  physeq@phy_tree$edge.length <- NULL
  expect_error(unifrac(physeq), "Unifrac distances require a tree with branch lengths, consider adding branch lengthes to the tree.")
  ## Missing tree
