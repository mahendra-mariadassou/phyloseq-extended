...
specificity <- function(x, index = c("shannon", "simpson", "yanai", "indspec"), groupfrac = NULL) {
  ...
  if (index == "indspec") {
    stopifnot(!is.null(groupfrac))
  }
  ## Change vector to matrix
  if (is.vector(x)) {
    x <- matrix(x, nrow = 1)
    if (index == "indspec") {
      groupfrac <- matrix(groupfrac, nrow = 1)
    }
  }
...
estimate_specificity <- function(physeq, group,
  ...
  if (se) {
    ...
    distrib <- t(apply(resmat, 1, function(x) {
      c(
        mean = mean(x), sd = sd(x),
...
estimate_local_specificity <- function(physeq, group,
  ...
  res <- data.frame(
    otu = rep(rownames(relative.abundance), ncol(relative.abundance)),
...
plot_specificity <- function(specOTUS, y = "specificity", color = "level",
                             se = TRUE, plot = TRUE) {
  p <- ggplot(specOTUS, aes_string(x = "abundance", y = y, color = color)) +
    geom_point(size = 2)
...
specific_rarefaction <- function(physeq, step = 10, group, index = c("fraction", "indspec"),
  ...
  if (is.null(specificity)) {
    specificity <- estimate_local_specificity(
      physeq = physeq, group = group,
      index = index, freq = TRUE,
      B = B, se = FALSE, parallel = parallel
    )
  }
  if (is.null(threshold)) {
    test_specificity <- test_local_specificity(
      physeq = physeq,
      group = group,
      index = index, freq = TRUE,
