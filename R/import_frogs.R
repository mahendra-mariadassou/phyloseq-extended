...
import_frogs <- function(biom,
  ...
  if (all(purrr::map_lgl(taxlist, is.null))) {
    ...
    ranknames <- c(
      ...
      paste0("Rank_", seq(from = 8, length.out = max(0, max_n_ranks - 7)))
    )[1:max_n_ranks]
    ## Empty tax table
    taxtab <- matrix(NA_character_,
      nrow = n_taxa, ncol = max_n_ranks,
      dimnames = list(taxnames, ranknames)
    )
    ## Fill tax table using two column index array
    ii <- cbind(
      rep(1:n_taxa, times = n_ranks), ## row index
  ...
  if (!is.null(refseqfilename)) {
    if (inherits(refseqfilename, "XStringSet")) {
      ...
      if (!is.null(refseqArgs)) {
        refseq <- do.call("refseqFunction", c(
          list(refseqfilename),
          refseqArgs
        ))
      } else {
