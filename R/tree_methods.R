...
daOTU_plot <- function(physeq, daOTUS, variable, title = NULL, plot = TRUE) {
  ...
  if (numberOfProvidedTaxa != length(daOTUS)) {
    cat(paste(
      length(daOTUS), "otus out of", numberOfProvidedTaxa,
      "provided were found, use only those for plotting"
    ), sep = "\n")
  }
  ## Get count data set
...
addThermo <- function(lastPP, meandf, adj.x, width.thermo = ncol(meandf) * 8 * strwidth(" ")) {
  ## Add Thermometer
  BOTHlabels(
    XX = max(lastPP$xx[1:lastPP$Ntip] + adj.x) + 0.3 * 8 * strwidth(" ") + 0.5 * width.thermo,
...
addAbundance <- function(otuAbun, lastPP, adj.x, width.thermo,
  ...
  if (abundance.type == "circle") {
    ...
    xcenter <- rep(
      max(lastPP$xx[1:lastPP$Ntip] + adj.x) + 0.5 * 8 * strwidth(" ") + width.thermo + adj.radii,
      lastPP$Ntip
    )
    ...
    XX <- rep(
      max(lastPP$xx[1:lastPP$Ntip] + adj.x) + 0.5 * 8 * strwidth(" ") + width.thermo,
      lastPP$Ntip
    ) + scaling / 2
    YY <- lastPP$yy[1:lastPP$Ntip]
    symbols(XX, YY,
      inches = FALSE, rectangles = cbind(scaling, 0.5),
  ...
  return(invisible(list(
    scale = scale, type = abundance.type,
    breaks = breaks
  )))
}
...
daOTU_tree_plot <- function(physeq, daOTUS, variable, colorTip = FALSE,
  ...
  daOTUS <- intersect(daOTUS, taxa_names(physeq))
  stopifnot(
    variable %in% sample_variables(physeq),
    length(daOTUS) > 0
  )
  if (numberOfProvidedTaxa != length(daOTUS)) {
    cat(paste(
  ...
  if (is.null(weight)) {
    ...
    if (!is.null(names(weight)) && all(groupLevels %in% names(weight))) {
      ...
      cat("No name or non-matching names for weight, using the first weights.",
        paste("Group names are", groupLevels),
        sep = "\n"
      )
...
ggtree <- function(physeq, group = NULL, freq = TRUE, method = "linear", base = 10,
  ...
  fattened.edges <- data.frame(
    edge.name = edge.names,
    t(fattened.edges)
  )
  mean.fattened.edges <- data.frame(
    edge.name = edge.names,
  ...
  if (lastPP == "cladogram") {
    p <- ggplot(ggdata) +
      geom_segment(aes(x = edge.x1, y = edge.y1, xend = edge.x2, yend = edge.y2))
  }
  if (lastPP == "phylogram") {
    p <- ggplot(ggdata) +
      geom_segment(aes(x = edge.x1, y = edge.y1, xend = edge.x1, yend = edge.y2))
